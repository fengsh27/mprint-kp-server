...
CSV_TO_DB_COLUMN_MAP = {
    ...
    "drug_list": "drug_list",
    "id_list": "id_list",
    "pediatric_keyword": "pediatric_keyword",
    "pediatric_mesh": "pediatric_mesh",
...
def create_table(cursor) -> None:
    ...
    create_sql = f"""
    CREATE TABLE IF NOT EXISTS pediatric_database_with_scores (
      {columns_sql},
...
def insert_rows(cursor, df: pd.DataFrame, batch_size: int) -> int:
    ...
    insert_sql = (
        f"INSERT IGNORE INTO pediatric_database_with_scores ({columns_sql}) "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Load pediatric_database_with_scores from csv file into MySQL."
    )
    parser.add_argument(
        "--csv",
        default="~/temp/pediatric_database_with_scores.csv",
    ...
    try:
        ...
        create_table(cursor)
        if args.truncate:
            cursor.execute("TRUNCATE TABLE pediatric_database_with_scores")
        total_inserted = 0
        total_dropped = 0
        for chunk in read_csv_chunks(args.csv, args.chunk_size):
            ...
            conn.commit()
        print(f"Inserted {total_inserted} rows into pediatric_database_with_scores.")
        if total_dropped:
