...
async function meshTermsSummaryHandler(req: Request) {
  ...
  try {
    ...
    const excludeKeywords = Array.isArray(body?.excludeKeywords)
      ? body.excludeKeywords.filter((item: unknown) => typeof item === "string")
      : [];
    ...
    const maternal = buildWordCloudList(meshTerms.maternal, excludeKeywords);
    const pediatric = buildWordCloudList(meshTerms.pediatric, excludeKeywords);
    ...
    const response = NextResponse.json({ maternal, pediatric });
    return addSecurityHeaders(response);
  } catch (error) {
    console.error("Error in mesh terms summary API:", error);
    logSecurityEvent(req as any, "DATABASE_ERROR", { error: error instanceof Error ? error.message : String(error) });

    return NextResponse.json(
      {
        error: "Internal Server Error",
        message: "Failed to fetch MeSH term summary"
      },
      { status: 500 }
    );
  }
...
export const POST = withRateLimit(meshTermsSummaryHandler, searchRateLimiter);
